...
nodes_fragment_shader = """
    ...
    void main()
    {
      ...
      if(d > 0.0) a = v_alpha * exp(-d*d*10000000.);
...
if __name__ == '__main__':
    ...
    node_positions = np.load('/Users/vincentherrmann/Documents/Projekte/Immersions/visualization/layouts/e25_version_3/e25_positions_interp_100.npy').astype(np.float32)
    x_max = 8.20502231383
    x_min = -8.8943710115
    y_max = 9.72496290916
    y_min = -9.50029536002
    node_positions[:, :, 0] = (node_positions[:, :, 0] - x_min) / (x_max - x_min)
    node_positions[:, :, 1] = (node_positions[:, :, 1] - y_min) / (y_max - y_min)
    ...
    edges_textures = np.load(
        '/Users/vincentherrmann/Documents/Projekte/Immersions/visualization/layouts/e25_version_3/e25_positions_interp_100_edges_texture.npy') * 1.
    edges_textures = edges_textures.astype(np.float32)
    edges_textures = np.power(edges_textures, 0.25)
    ...
    # widget.setLayout(qtpy.QtWidgets.QVBoxLayout())
    # widget.layout().addWidget(c.native)
    window.show()
